...
- [ ] `canonicalGraphJson` in `@flowlathe/core`, order-insensitive, unit-tested.
- [ ] `flow_versions` carries `content_hash` (unique per flow), `label`, `message`,
      `parent_version_id`; migration generated by `drizzle-kit generate` and backfilled safely.
- [ ] `saveFlowVersion` deduplicates; an unchanged save creates no row; labeling is idempotent.
- [ ] `diffGraphs` + `isSemanticChange` in `@flowlathe/core`, with reparent, handle-change, and
      template-line-diff coverage.
- [ ] Version history API + canvas drawer, diff view, "Save as version…", restore-as-new-head.
- [ ] Execution views name the version they ran and link to a read-only graph and a diff vs. current.
- [ ] Step-mode staleness chip, with a test that the underlying HEAD-following behavior is unchanged.
- [ ] Retention GC with all four guards, configurable, logged.
- [ ] `flow_pins` + pin UI; triggers pin explicitly and never follow HEAD.
- [ ] (After the DSL plan) read-only git history tab reusing `diffGraphs`.
- [ ] README section on what a version is (revision vs. named version vs. pin); `CLAUDE.md` updated.
