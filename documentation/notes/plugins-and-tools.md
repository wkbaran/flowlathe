...
- **PLAN-DOMAIN-TOOLS.md / PLAN-GIT.md added `@flowlathe/plugin-git`, the first "Tier B" plugin**
  (a real subprocess with a fixed argv template per tool, rather than an HTTP client) — following
  `PLAN-DOMAIN-TOOLS.md`'s prepared-statement framing: the plugin author writes the whole argv
  except for named, typed value slots; a model never supplies a program, subcommand, or flag.
  - **This plugin never runs `git config` or `git remote`, and must never learn to.** Those two
    subcommands are the entire reason local `git` is hard-refused from ever being reachable
    through a generic command-allowlist shell tool: `git config core.pager 'sh -c ...'` followed
    by any later `git` invocation that pages is arbitrary code execution in two calls, with no
    flag on any blocklist and no interpreter ever allowlisted. `git` is not, and will not become,
    reachable through this repository's (unbuilt) shell/exec toolset for exactly this reason — see
    `PLAN-DOMAIN-TOOLS.md` §1.1.
  - **The model never supplies a subcommand or a flag — `commands.ts` owns every argv template,
    and a model argument can only land in a named, validated slot.** `commands.test.ts` (asserting
    the exact argv array for all ten tools, across every present/absent combination of optional
    slots) is what actually enforces this and is worth more than the rest of the suite combined —
    it's the one file that would catch a future edit accidentally leaking a flag into a slot.
  - **`git_diff` builds its `from..to` range from two independently validated refs**, never from
    one model-supplied string — a single `"a..b"` argument would be one edit away from a model
    supplying `"a --output=/etc/x"` instead.
  - **`HOME` and `PATH` are inherited here, deliberately inverting the shell-tool plan's own rule**
    of building a minimal, sandboxed environment. This looks like a mistake on first read — it
    isn't: git needs `~/.gitconfig`, the credential helper, and `ssh` to function at all, and that
    configuration is entirely the operator's own, set up by hand outside flowlathe. That's exactly
    what makes inheriting it safe here specifically: this plugin can never invoke the one thing
    that would turn "the config is readable" into "the config is a code-execution primitive" —
    `git config`/`git remote` are the tools that don't exist. Everything else is still stripped to
    an explicit small set (`LANG`, `TZ`, `SSH_AUTH_SOCK`, `GIT_TERMINAL_PROMPT`, `GIT_PAGER`,
    `GIT_OPTIONAL_LOCKS`) — a git hook is arbitrary code running with this same environment, and
    `config.test.ts` plants `FIRECRAWL_API_KEY`/`GITHUB_TOKEN` in the parent process and asserts
    neither name reaches `cfg.env` to keep it that way.
  - **`GIT_TERMINAL_PROMPT=0` prevents a hang, not just an untidy prompt.** Without it, a `git
    push` against a remote with unusable/expired credentials blocks on an interactive terminal
    prompt that will never be answered, until the subprocess timeout eventually fires (120s for
    push) — the flow just looks stuck, not failed, for two minutes.
  - **Sanitize after splitting records, never before.** `git log -z`'s NUL record terminator and
    the `%x1f` unit-separator field delimiter are both control characters; running
    `scrubUntrustedText` (which strips control characters) before splitting on them would collapse
    every record and every field into one. This is the single most likely bug shape in this
    plugin — get the order backwards and the parser doesn't crash, it silently returns nonsense.
  - **`git branch --format` (the `for-each-ref` engine) does not understand `%x1f`, the hex-escape
    syntax `git log`/`git show --format` (the pretty-format engine) use — it uses a bare `%1f`
    instead, with no `x`.** `PLAN-GIT.md`'s own `git_list_branches` template as written uses
    `%x1f`; verified against a real git 2.43 repository that this is not rejected and not a no-op —
    it is emitted completely literally as the four characters `%x1f` in the output, which would
    have silently corrupted every field split in production. Caught only because `tools.test.ts`
    asserts against real parsed output rather than just "the tool didn't throw." Fixed in
    `commands.ts`'s `listBranchesArgv`, which also folds in `%(HEAD)` (`"*"` for the current
    branch) so `current` doesn't need a second subprocess call. If a `git branch`/`git tag`/`git
    for-each-ref` argv is ever added elsewhere in this codebase, remember this split: `%x1f` is a
    `git log`/`git show`-family thing only.
  - **`git commit` with nothing staged writes its explanation to STDOUT, not stderr, with a
    non-zero exit and an EMPTY stderr** — verified against a real git 2.43 repo. Every other
    failure this plugin produces (an invalid ref, a missing branch) does use stderr, so a
    stderr-only failure-message extractor would silently return an empty string for this one,
    very common case. `tools.ts`'s `extractGitMessage` falls back to stdout when stderr is empty.
  - **A non-zero exit is a successful tool call, always** — returned as `toolFail` carrying git's
    own message (via `extractGitMessage`'s stderr-then-stdout fallback above), never thrown. A
    `guarded`-classified failure is reserved for "the tool could not run git at all" (the binary
    missing, a spawn-level failure) — conflating the two would mean a model can't read "nothing to
    commit, working tree clean" as an ordinary, actionable result.
  - **No golden parity fixture and no Playwright e2e spec, both tracked gaps, not hidden ones.**
    The parity harness stubs the mock provider and outbound HTTP (`net-stub.ts`) but has no stub
    for a subprocess; a fixture that really shells out to `git` would make the harness
    environment-dependent for the first time, and doing it properly needs a
    `(nodeId, sha256(argv))`-keyed stub table mirroring `injectResponseTable` — the same design
    PLAN-INTEGRATIONS.md's design trap 8 already describes for the deferred `fetch`-node fixture.
